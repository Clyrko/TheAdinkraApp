...
extension AttributedText{
    ...
    func attributing(_ attribute:AttributedText.Attribute)->AttributedText{
        ...
        case .style(let style):
            attrs.updateValue(style.font, forKey: .font)
        case .font(let font):
...
extension AttributedText{
    ...
    struct TextShadow{
        ...
        static func dropShadow(offset:CGSize = .init(width: .zero, height: 2), blur:CGFloat = 2, color:UIColor = .black, opacity:CGFloat = 0.6)->TextShadow{
            let shadow = NSShadow()
