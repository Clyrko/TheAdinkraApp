...
extension CategoryDetailsViewController: UITableViewDataSource, UITableViewDelegate {
    func tableView(_ tableView: UITableView, numberOfRowsInSection section: Int) -> Int {
        ...
        case "Love":
            return loveSymbols.count
        case "Power":
            return powerSymbols.count
        case "Wealth":
            return wealthSymbols.count
        case "Wisdom":
            return wisdomSymbols.count
        case "Home":
            return homeSymbols.count
        case "Peace":
            return peaceSymbols.count
        case "War":
            return warSymbols.count
        case "Faith":
            return faithSymbols.count
        case "Self":
            return selfSymbols.count
        case "Nature":
            return natureSymbols.count
        default:
            break
        }
        return 1
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        case "Love":
            ...
            cell.setup(with: loveSymbols[indexPath.row])
            return cell
        case "Power":
            guard let cell = tableView.dequeueReusableCell(withIdentifier: CategorySymbolCell.Identifier, for: indexPath) as? CategorySymbolCell else { fatalError() }
            cell.setup(with: powerSymbols[indexPath.row])
            return cell
        case "Wealth":
            guard let cell = tableView.dequeueReusableCell(withIdentifier: CategorySymbolCell.Identifier, for: indexPath) as? CategorySymbolCell else { fatalError() }
            cell.setup(with: wealthSymbols[indexPath.row])
            return cell
        case "Wisdom":
            guard let cell = tableView.dequeueReusableCell(withIdentifier: CategorySymbolCell.Identifier, for: indexPath) as? CategorySymbolCell else { fatalError() }
            cell.setup(with: wisdomSymbols[indexPath.row])
            return cell
        case "Home":
            guard let cell = tableView.dequeueReusableCell(withIdentifier: CategorySymbolCell.Identifier, for: indexPath) as? CategorySymbolCell else { fatalError() }
            cell.setup(with: homeSymbols[indexPath.row])
            return cell
        case "Peace":
            guard let cell = tableView.dequeueReusableCell(withIdentifier: CategorySymbolCell.Identifier, for: indexPath) as? CategorySymbolCell else { fatalError() }
            cell.setup(with: peaceSymbols[indexPath.row])
            return cell
        case "War":
            guard let cell = tableView.dequeueReusableCell(withIdentifier: CategorySymbolCell.Identifier, for: indexPath) as? CategorySymbolCell else { fatalError() }
            cell.setup(with: warSymbols[indexPath.row])
            return cell
        case "Faith":
            guard let cell = tableView.dequeueReusableCell(withIdentifier: CategorySymbolCell.Identifier, for: indexPath) as? CategorySymbolCell else { fatalError() }
            cell.setup(with: faithSymbols[indexPath.row])
            return cell
        case "Self":
            guard let cell = tableView.dequeueReusableCell(withIdentifier: CategorySymbolCell.Identifier, for: indexPath) as? CategorySymbolCell else { fatalError() }
            cell.setup(with: selfSymbols[indexPath.row])
            return cell
        case "Nature":
            guard let cell = tableView.dequeueReusableCell(withIdentifier: CategorySymbolCell.Identifier, for: indexPath) as? CategorySymbolCell else { fatalError() }
            cell.setup(with: natureSymbols[indexPath.row])
            return cell
        default:
            break
        }
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        switch categories.category {
        case "Love":
            controller.symbols = loveSymbols[indexPath.row]
            navigationController?.pushViewController(controller, animated: true)
        case "Power":
            controller.symbols = powerSymbols[indexPath.row]
            navigationController?.pushViewController(controller, animated: true)
        case "Wealth":
            controller.symbols = wealthSymbols[indexPath.row]
            navigationController?.pushViewController(controller, animated: true)
        case "Wisdom":
            controller.symbols = wisdomSymbols[indexPath.row]
            navigationController?.pushViewController(controller, animated: true)
        case "Home":
            controller.symbols = homeSymbols[indexPath.row]
            navigationController?.pushViewController(controller, animated: true)
        case "Peace":
            controller.symbols = peaceSymbols[indexPath.row]
            navigationController?.pushViewController(controller, animated: true)
        case "War":
            controller.symbols = warSymbols[indexPath.row]
            navigationController?.pushViewController(controller, animated: true)
        case "Faith":
            controller.symbols = faithSymbols[indexPath.row]
            navigationController?.pushViewController(controller, animated: true)
        case "Self":
            controller.symbols = selfSymbols[indexPath.row]
            navigationController?.pushViewController(controller, animated: true)
        case "Nature":
            controller.symbols = natureSymbols[indexPath.row]
            navigationController?.pushViewController(controller, animated: true)
        default:
            break
        }
